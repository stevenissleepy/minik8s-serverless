...
curl -s http://localhost:30082/api/v1/namespaces/default/services/sentiment-it/state | jq
```
...
kubectl get serverlessservices
kubectl get revisions
curl -s http://localhost:8080/apis/serverless.minik8s.io/v1alpha1/namespaces/default/serverlessservices/sentiment-it | jq '.spec, .status'
```
...
kubectl get service
kubectl get pods -l serverless.minik8s.io/service=sentiment-it -o wide
curl -s http://localhost:8080/apis/serverless.minik8s.io/v1alpha1/namespaces/default/serverlessservices/sentiment-it | jq '.spec.image, .status'
curl -s http://localhost:30082/api/v1/namespaces/default/services/sentiment-it/state | jq
...
curl -s "http://localhost:30082/api/v1/namespaces/default/workflows/ticket-router/invoke" \
  -H 'content-type: application/json' \
  -d '{"ticket_id":"T-001","user_level":"normal","text":"password login error"}' | jq

# 通过事件触发 workflow，高风险工单走 human-escalate
curl -s "http://localhost:30082/api/v1/events/ticket.created" \
  -H 'content-type: application/json' \
...
- `kubectl get workflows` 能看到 `ticket-router`
- `kubectl get eventtriggers` 能看到 `ticket-created`。
...
```sh
curl -s "http://localhost:30082/api/v1/namespaces/default/services/risk-score/invoke" \
  -H 'content-type: application/json' \
...
kn func deploy risk-score \
  --image ghcr.io/stevenissleepy/risk-score:v2 \
  --api-server http://localhost:8080
...
- 更新后 `ServerlessService/risk-score` 的 `.spec.image` 为 `ghcr.io/stevenissleepy/risk-score:v2`。
- `kubectl get revisions` 能看到新的 `risk-score-*` revision。
- 更新后的调用结果包含 `"model_version": "risk-v2"`，且 `risk` 固定为 `100`、`decision` 固定为 `human`。
...
seq 1 20 | xargs -P20 -I{} sh -c '
  ...
  while [ "$(date +%s)" -lt "$end" ]; do
    ...
    curl -s "http://localhost:30082/api/v1/namespaces/default/services/risk-score/invoke" \
      -H "content-type: application/json" \
      -d "{\"ticket_id\":\"LOAD-$worker-$n\",\"user_level\":\"vip\",\"category\":\"complaint\",\"text\":\"angry urgent security leak\",\"sleep_ms\":500}" \
      > "/tmp/risk-score-load/$worker-$n.json"
  done
' sh {} &
load_pid=$!
```
...
for i in 1 2 3 4; do
  ...
  curl -s "http://localhost:30082/api/v1/namespaces/default/services/risk-score/state" | jq '.runtime'
done

wait "$load_pid"

jq -s 'length as $total | map(select(.result.model_version == "risk-v2" and .result.risk == 100)) | length as $ok | {total: $total, ok: $ok}' \
  /tmp/risk-score-load/*.json
jq -r '.result.instance' /tmp/risk-score-load/*.json | sort | uniq -c
curl -s "http://localhost:30082/api/v1/namespaces/default/services/risk-score/state" | jq '.runtime'
```
...
- `jq -s ...` 的 `ok` 应等于 `total`，说明请求都由 v2 处理并返回 `risk: 100`。
- `jq -r '.result.instance' ... | sort | uniq -c` 会显示请求落到哪些实例上；出现多个 `instance` 就说明请求被多个 runtime Pod 处理。
- `state.runtime.active_instances` 通常会大于 `1`，最多到 `maxScale: 4`。
...
curl -i -s "http://localhost:30082/api/v1/namespaces/default/services/auto-reply/invoke" \
  -H 'content-type: application/json' \
  -d '{"ticket_id":"D-001","category":"technical","risk":20,"decision":"auto"}'

curl -i -s "http://localhost:30082/api/v1/namespaces/default/workflows/ticket-router/invoke" \
  -H 'content-type: application/json' \
